...
/**
 * 
 * @author Antonin Liste d'actionPoids qui contient les différentes actions
 */
public class ListActionPoids extends ArrayList<ActionPoids> {
	...
	public void cheminPossible(Agent a, BomberManGame game) {
		if (game.IsLegalMove(a, AgentAction.MOVE_DOWN))
			this.add(new ActionPoids(AgentAction.MOVE_DOWN, 10));
		if (game.IsLegalMove(a, AgentAction.MOVE_UP))
			this.add(new ActionPoids(AgentAction.MOVE_UP, 10));
		if (game.IsLegalMove(a, AgentAction.MOVE_RIGHT))
			this.add(new ActionPoids(AgentAction.MOVE_RIGHT, 10));
		if (game.IsLegalMove(a, AgentAction.MOVE_LEFT))
			this.add(new ActionPoids(AgentAction.MOVE_LEFT, 10));
		if (game.IsLegalMove(a, AgentAction.PUT_BOMB))
			this.add(new ActionPoids(AgentAction.PUT_BOMB, 10));
	...
	public void eviterObjects(Agent a, BomberManGame game, ArrayList<Position> objectToAvoid, int poid) {
		...
		for (Position pos : objectToAvoid) {
			if (a.getX() == pos.getX() && a.getY() == pos.getY()) {
				if (this.contains(AgentAction.PUT_BOMB))
					this.change(AgentAction.PUT_BOMB, -1);
			} else {
	...
	public void moveto(Agent a, BomberManGame game, Position pos, int poid) {
		if (pos != null) {
			...
			int distY = a.getY() - pos.getY();
			int poids = (int) Math.sqrt(Math.pow((distX), 2) + Math.pow(distY, 2));
			if (distX <= 1 && distY <= 1 && distX >= -1 && distY >= -1 && game.IsLegalMove(a, putBomb))
				change(putBomb, this.getPoids(putBomb) + distance - poids + 10);
			else {
				if (distX == 0) {
					moveUp(a, game, distance - poids + poid);
					moveDown(a, game, distance - poids + poid);
